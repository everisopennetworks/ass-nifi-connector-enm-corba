package org.omg.CosNotifyComm;


/**
* org.omg.CosNotifyComm/SequencePushSupplierHelper.java .
* Generated by the IDL-to-Java compiler (portable), version "3.2"
* from org.omg.CosNotifyComm.idl
* jueves 5 de septiembre de 2019 02:16:25 AM CDT
*/


// SequencePullSupplier
abstract public class SequencePushSupplierHelper
{
  private static String  _id = "IDL:omg.org/CosNotifyComm/SequencePushSupplier:1.0";

  public static void insert (org.omg.CORBA.Any a, SequencePushSupplier that)
  {
    org.omg.CORBA.portable.OutputStream out = a.create_output_stream ();
    a.type (type ());
    write (out, that);
    a.read_value (out.create_input_stream (), type ());
  }

  public static SequencePushSupplier extract (org.omg.CORBA.Any a)
  {
    return read (a.create_input_stream ());
  }

  private static org.omg.CORBA.TypeCode __typeCode = null;
  synchronized public static org.omg.CORBA.TypeCode type ()
  {
    if (__typeCode == null)
    {
      __typeCode = org.omg.CORBA.ORB.init ().create_interface_tc (SequencePushSupplierHelper.id (), "SequencePushSupplier");
    }
    return __typeCode;
  }

  public static String id ()
  {
    return _id;
  }

  public static SequencePushSupplier read (org.omg.CORBA.portable.InputStream istream)
  {
    return narrow (istream.read_Object (_SequencePushSupplierStub.class));
  }

  public static void write (org.omg.CORBA.portable.OutputStream ostream, SequencePushSupplier value)
  {
    ostream.write_Object ((org.omg.CORBA.Object) value);
  }

  public static SequencePushSupplier narrow (org.omg.CORBA.Object obj)
  {
    if (obj == null)
      return null;
    else if (obj instanceof SequencePushSupplier)
      return (SequencePushSupplier)obj;
    else if (!obj._is_a (id ()))
      throw new org.omg.CORBA.BAD_PARAM ();
    else
    {
      org.omg.CORBA.portable.Delegate delegate = ((org.omg.CORBA.portable.ObjectImpl)obj)._get_delegate ();
      _SequencePushSupplierStub stub = new _SequencePushSupplierStub ();
      stub._set_delegate(delegate);
      return stub;
    }
  }

  public static SequencePushSupplier unchecked_narrow (org.omg.CORBA.Object obj)
  {
    if (obj == null)
      return null;
    else if (obj instanceof SequencePushSupplier)
      return (SequencePushSupplier)obj;
    else
    {
      org.omg.CORBA.portable.Delegate delegate = ((org.omg.CORBA.portable.ObjectImpl)obj)._get_delegate ();
      _SequencePushSupplierStub stub = new _SequencePushSupplierStub ();
      stub._set_delegate(delegate);
      return stub;
    }
  }

}
